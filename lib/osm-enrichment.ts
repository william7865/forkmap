...
export interface OsmEnrichedData {
  ...
  // Classification
  stars?:         number;    // hotel/restaurant stars
  michelin?:      number;    // michelin stars
  organic?:       boolean;
  ...
  // Cuisine sub-types
  diet?:          string[];   // vegetarian, vegan, halal, etc.

  // Payment
  payment_methods?: string[]; // cash, card, contactless, etc.
...
export function extractOsmEnrichment(tags: OsmTags): OsmEnrichedData {
  ...
  result.booking_url = tags["reservation"] !== "no" ? (tags["booking"] ?? tags["reservation:url"]) : undefined;
  ...
  if (stars) { const n = Number(stars); if (!isNaN(n)) result.michelin = n; }
  ...
  // Diet
  const diets: string[] = [];
  if (tags["diet:vegetarian"] === "yes" || tags["diet:vegetarian"] === "only") diets.push("vegetarian");
  ...
  if (cap) { const n = Number(cap); if (!isNaN(n) && n > 0) result.capacity = n; }
