 ...
 */
export function computeScore(place: PlaceCard, maxDistanceM = 2000): number {
  const ratingScore = place.fsq?.rating != null ? place.fsq.rating / 10 : 0.5
  const popularityScore =
    place.fsq?.total_ratings != null ? Math.min(place.fsq.total_ratings / 500, 1) : 0.3
  const distanceScore =
    place.distance != null ? Math.max(0, 1 - place.distance / maxDistanceM) : 0.5
  ...
  const michelinBonus =
    (place.osm_enriched?.michelin ?? place.wikidata?.michelin_stars ?? 0) > 0 ? 0.2 : 0
...
export function applyFilters(places: PlaceCard[], filters: FilterState): PlaceCard[] {
  ...
  if (filters.minRating != null) {
    result = result.filter((p) => p.fsq?.rating == null || p.fsq.rating >= filters.minRating!)
  }
  ...
  if (filters.maxPrice != null) {
    result = result.filter((p) => p.fsq?.price == null || p.fsq.price <= filters.maxPrice!)
  }
  ...
  if (filters.maxDistance != null) {
    result = result.filter((p) => p.distance == null || p.distance <= filters.maxDistance!)
  }
