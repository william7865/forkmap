      ...
      <div style={{ padding: '5px 0' }}>
        {[
          { href: '/account', label: 'Mon compte', icon: <IcoUser /> },
          {
            ...
            label: 'Mes favoris',
            icon: (
              <span style={{ color: 'var(--forest-mid)', display: 'flex' }}>
                <IcoHeart />
              </span>
            ),
          },
...
export default function AuthButton({ auth, onOpenModal }: Props) {
  ...
  return (
    <div ref={triggerRef} style={{ position: 'relative', flexShrink: 0 }}>
      ...
      >
        ...
        >
          {avatarUrl ? (
            ...
            <span
              style={{ fontSize: 10, fontWeight: 600, color: 'white', letterSpacing: '-0.02em' }}
            >
              {initials}
            </span>
          )}
      ...
      {open && mounted && <DropdownPanel anchorRef={triggerRef} auth={auth} onClose={close} />}
    </div>
  )
}
