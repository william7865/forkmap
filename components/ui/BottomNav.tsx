    ...
    <>
      <nav style={{
        ...
        >
          ...
          <span style={{ fontSize: 10, fontWeight: 600, color: "var(--ink-40)", fontFamily: "var(--font-body)" }}>Plus</span>
        </button>
      </nav>
          ...
          >
            ...
            {MORE_LINKS.map(l => (
              <Link key={l.href} href={l.href} onClick={() => setSheet(false)} style={{
                ...
                fontSize: 15, color: "var(--ink-80)", textDecoration: "none",
                fontFamily: "var(--font-body)",
            ...
            {auth.user && (
              <button
                onClick={async () => {
                  try { await sb.auth.signOut(); } catch { /* ignore */ } finally { setSheet(false); }
                }}
                style={{
                  display: "block", width: "100%", padding: "14px 24px", textAlign: "left",
                  fontSize: 15, color: "var(--coral)", background: "none", border: "none",
