...
// The map lives at `/` in the native app but at `/carte` on the web (where `/`
// is the marketing landing). These watchers strip the consumed query param and
// stay on whatever route they're currently on — never a hardcoded `/`, which on
// the web would bounce the map back to the landing.
...
function SelectParamWatcher({ onSelect }: { onSelect: (osmId: string) => void }) {
  ...
  useEffect(() => {
    ...
    const qs = params.toString()
    router.replace(qs ? `${pathname}?${qs}` : pathname, { scroll: false })
  }, [searchParams, onSelect, router, pathname])
  return null
}
              ...
              {selectedPlace && (
                <button
                  ...
                  style={{
                    ...
                    border: `1px solid ${pinDropActive ? 'rgba(25,28,29,0.3)' : 'var(--border)'}`,
