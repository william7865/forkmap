...
// ── Icons ─────────────────────────────────────────────────
const IcoX = () => (
  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round"><path d="M18 6 6 18M6 6l12 12"/></svg>
);
const IcoPen = () => (
...
export default function NoteModal({ place, onClose, onSaved }: Props) {
  ...
  return (
    <div style={{ position:"fixed",inset:0,zIndex:9100,display:"flex",alignItems:"center",justifyContent:"center",padding:20 }}
      onClick={onClose}>
      <div style={{ position:"absolute",inset:0,background:"rgba(14,14,13,0.5)",backdropFilter:"blur(8px)" }}/>
      <div onClick={e=>e.stopPropagation()} style={{
        position:"relative", background:"var(--white)", borderRadius:"var(--r-xl)",
        ...
        <div style={{
          ...
          <div style={{
            ...
            background:"var(--forest-pale)", border:"1px solid rgba(45,122,85,0.2)",
            display:"flex", alignItems:"center", justifyContent:"center",
            flexShrink:0, color:"var(--forest-mid)",
          }}>
            <IcoPen />
          </div>
          <div style={{ flex:1, minWidth:0 }}>
            <h2 style={{
              ...
              fontSize:17, fontWeight:400, letterSpacing:"-0.03em",
          ...
          <button onClick={onClose} style={{
            width:28, height:28, borderRadius:"50%", border:"1px solid var(--ink-10)",
            background:"var(--off-white)", color:"var(--ink-60)", cursor:"pointer",
        ...
        <div style={{ padding:"16px 20px 20px" }}>
          ...
          <div style={{ position:"relative" }}>
            <textarea
              ...
              onBlur={e=>{e.target.style.borderColor=value.length > MAX * 0.9 ? "var(--amber)" : "var(--ink-10)"; e.target.style.boxShadow="none";}}
            />
            <span style={{
              position:"absolute", bottom:10, right:12,
              fontSize:10, color: value.length > MAX * 0.9 ? "var(--amber)" : "var(--ink-40)",
          ...
          <div style={{ display:"flex", gap:8, marginTop:14 }}>
            {value.trim() && (
              <button onClick={() => { setValue(""); saveNote(place.osm_id, ""); }} style={{
                padding:"9px 14px", borderRadius:"var(--r-md)",
                border:"1px solid rgba(217,79,61,0.25)", background:"transparent",
                color:"var(--coral)", cursor:"pointer", fontSize:12, fontWeight:600,
