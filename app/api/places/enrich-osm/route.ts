...
export async function POST(req: NextRequest) {
  ...
  const enriched = await Promise.all(
    (places as PlaceCard[]).map(async (place) => {
      ...
      // 5. Mapillary storefront — last-resort free photo, only for places that
      // still have none (no Google/FSQ photo, no OSM/Commons image, no Wikidata
      // image). Cached 30 days server-side; degrades to null without a token.
      const hasPhoto =
