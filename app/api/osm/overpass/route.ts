...
export async function GET(req: NextRequest): Promise<NextResponse<OverpassApiResponse>> {
  ...
  const limited = rateLimit(req, {
    limit: 30,
    windowMs: 60_000,
    message: 'Too many map requests. Slow down a little!',
  })
  ...
  try {
    const { data: places, cached } = await cacheAside(bbox_key, CACHE_TTL, () =>
    ...
    res.headers.set(
      'Cache-Control',
      cached
        ? `public, max-age=${CACHE_TTL}, stale-while-revalidate=120`
